...
def search_cards():
    ...
    print data['name']
    if not data['name'] == "":
        card = db.session.query(Card).filter((Card.name).ilike \
        ("%" + data['name'] + "%")).all()
...
def get_card():
    ...
    card = db.session.query(Card).filter(func.lower(Card.name).ilike \
   (func.lower("%" + data['name'] + "%"))).first()
    try:
...
def cycle_editions():
    ...
    print data['edition']
    card = db.session.query(Card).filter(Card.name == data['name']).filter \
        (Card.edition_id > data['edition']).first()
    if card:
        return jsonify(card.card_info())
    else:
